...
- **Phase 1 foundation:** organizations may store branding fields (`logoUrl`, `tagline`, `primaryColor`, `secondaryColor`, `accentColor`) on the org / `organizationProfile` — editable in Organization Setup / onboarding
- **Phase 2:** full workspace chrome driven by those branding fields (theme, logo in shell, etc.)
